...
class DeconvLayer(object):
    ...
    def __call__(self, inputs):
        ...
        # Define bias tensor as well using zero initializer                                        #
        ############################################################################################
        self.deconv_filter = normal_initializer(
            shape=(self.kernel_size, self.kernel_size, self.output_filters, self.input_filters))
        ...
        # Finally apply activation function and store it as self.total_output   #
        #########################################################################
        self.deconv_output = tf.nn.conv2d_transpose(inputs, filter=self.deconv_filter,
                                                    output_shape=[10, output_height, output_width,
                                                                  self.output_filters],
                                                    strides=[1, self.kernel_stride, self.kernel_stride, 1],
                                                    padding=self.kernel_padding)
        # Add bias and apply activation
