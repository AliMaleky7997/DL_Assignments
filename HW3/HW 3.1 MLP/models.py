...
class Dense(object):
    ...
    def _build(self):
        for i in range(1, len(self.num_hidden)):
            ...
            layer = DenseLayer(input_dim=self.num_hidden[i - 1], output_dim=self.num_hidden[i], act=act,
    ...
    def _loss(self):
        ...
        batch_loss = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.activations[-1],
                                                                labels=self.placeholders['batch_labels'])

        # L2 regularization on weights
        l2_loss = FLAGS.weight_decay * (tf.nn.l2_loss(self.layers[0].vars['weight']) + tf.nn.l2_loss(
            self.layers[1].vars['weight']))
